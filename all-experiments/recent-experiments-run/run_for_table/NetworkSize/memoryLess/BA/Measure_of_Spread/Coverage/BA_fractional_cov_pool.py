...
def plotGraph(algorithm_1,algorithm_2,fileName,xplotName,jurySize,xplot):
    ...
    with open('frac_cover_ba_memless.txt', 'a') as file:
...
def variationOfPoolSize(xplot,graph_param_list, jurySize, fileName,xplotName):
    ...
    for obj in graph_param_list:
                ...
                obj.getParams()
                fractNodeCovList_01[itr] = variationRunAlgo(1,jurySize,graph,0.25,2)
                fractNodeCovList_02[itr] = variationRunAlgo(2,jurySize,graph,0.25,2)
                itr = itr + 1
...
variationOfPoolSize(xplot,graph_param_list,jurySize,"screenshots_fractional_cov/fractional_cov_network_size.jpg",xplotName)
