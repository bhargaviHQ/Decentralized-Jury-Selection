...
def variationOfSeedSize(xplot,graph_param_list, jurySize, fileName):
    ...
    itr = 0
    list_names = [f"jurySize={i}" for i in xplot]
    df1 = pd.DataFrame()
    df2 = pd.DataFrame()
    ...
    plt.title("Betweenness Centrality Strategy for varying Jury size")
    plt.savefig(fileName+"_between.jpg")

    txtfile1 = fileName+'_between_memless.txt'
    max_values = df1.max()
    minval1 = df1.min()
    ...
    plt.xlabel("Varying Jury size")
    plt.ylabel("Probabilities of Occurrence")
    plt.title("Coverage Strategy for Jury size")
    plt.savefig(fileName+"_coverage.jpg") 

    txtfile2 = fileName+'_coverage_memless.txt'  
    max_values2 = df2.max()
    minval2 = df2.min()
...
xplot = [6,12,25,50]            
variationOfSeedSize(xplot,graph_param_list,jurySize,"screenshots_disparity/disparity_jurysize")
