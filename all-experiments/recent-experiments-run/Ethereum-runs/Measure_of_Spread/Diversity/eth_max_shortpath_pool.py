...
def plotGraph(algorithm_1,algorithm_3,fileName,xplotName,xplot):
    ...
    with open('ETH_POOL_max_short_path_ba_memenhanced.txt', 'a') as file:
        file.write(str("\n"))
        file.write(str(algorithm_1))

    with open('ETH_Random_max_short_path_ba_memenhanced.txt', 'a') as file:
        file.write(str("\n"))
...
def variationRunAlgo(algoIndex, jurySize, graph):
    ...
    if algoIndex == 1:
        ...
        for i in range (0,iterations):
            ...
            avgDistList[itr]  = maximum_shortest_path.maximumShortestPathLargeTable(graph, selectedSet)
            itr = itr + 1
